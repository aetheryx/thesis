...
Uber's CDE is a cloud-based application used by engineers to develop codebases in monorepos. These CDE's are currently provisioned with a storage device known as a Persistent Disk. The cloud provider has recently announced a new type of disk known as a Hyperdisk. Uber would like to investigate a migration from Persistent Disks to Hyperdisks: they need a thorough understanding of the differences between the disk types, including what their fundamental differences are, how the practical performance of CDE's is impacted by disk performance, and the effectiveness of Hyperdisk-specific features (such as storage pooling and snapshot speed).
