...
= Introduction
== Introduction
This section describes various contextual details for the research. Namely, the application is introduced and its relation to disk performance is described. Further, the problem statement is mentioned, followed by the main question and sub questions. 

== Context of the application
Since the 2000s, a number of large tech companies have adopted the concept of "monorepos". A monorepo is defined as a software development strategy in which distinct projects are stored in the same repository. Monorepos offer a number of key advantages, such as improved dependency management and code re-use. Within a monorepo, projects can form direct dependencies on each other and re-use the same core libraries. However, monorepos often come with significant scaling challenges, as they often scale up to thousands of projects and billions of lines of code. 
...
In order to solve this problem, Uber has developed a cloud-based development environment. The key advantage of cloud-based development environments is that the cloud offers significantly more powerful hardware than what is found in laptops. Uber's cloud development is used daily by more than 4,000 software engineers.

== Relevance to disk performance
As previously mentioned, Uber has adopted the concept of monorepos for the vast majority of their codebases, and a key characteristic of monorepos are direct dependencies: projects within monorepos can depend on other projects within the same monorepo.

In order for monorepos to remain performant, it is important that dependencies are cached. This is especially relevant in compiled languages: if a given project has a number of dependencies, and the source code of the project changes, it should only be necessary to recompile the source code of the project and not it's dependencies. When dependencies are cached, they only need to be compiled the first time the project is built.
