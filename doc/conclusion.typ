...
= Conclusion
An answer to the main question can now be derived from the answers to each of the sub-questions.
...
The next chapter, @build_perf, investigates how the build duration of projects in monorepos is impacted by Hyperdisks. Despite the fact that Hyperdisks reduce raw disk performance, build duration proved to be minimally impacted: across the monorepos, Hyperdisks resulted in an average 0,9% increase in build duration. This is due to the fact that the monorepos can be configured to use ephemeral storage for disk-heavy workloads. In conclusion, Hyperdisks minimally impact the build duration of projects in monorepos. 

Lastly, chapter @disk_cap investigates how Hyperdisk Storage Pools affect disk utilization. Thin provisioning was estimated to reduce total storage capacity significantly, by up to 68,5%. Data reduction was determined to be minimally relevant to Uber's CDEs. Lastly, a subset of production CDEs was cloned and provisioned in a storage pool, in order to predict capacity reduction in practice. A 68,28% reduction in total capacity was observed, improving storage utilization from 30,74% to 96,91%. In conclusion, Hyperdisk Storage Pools significantly improve storage utilization for Uber's CDE fleet, reducing total storage capacity by up to 68,5%.

To conclude: by adopting Hyperdisks for Uber's CDE, raw disk performance is significantly reduced, build duration is minimally impacted, and disk capacity utilization is significantly improved. Overall, adoption of Hyperdisks is a sensible strategy. Firstly, the storage utilization improvements result in significant cost savings. Secondly, the fact that build duration is minimally impacted implies that the majority of users will not be affected negatively. And lastly, for the users who are negatively affected by the reduction in raw performance, their problems can be mitigated by provisioning extra performance for them, which can be paid for using the cost savings from the capacity optimisations.