...
In order to demonstrate burstability, we will perform an experiment with the goal of understanding how execution time is impacted by the amount of active @cde:short:pl. The task that will be measured is writing 4 GiB of randomized data to the disk. For each CDE group, we will measure the task execution time, with a varying amount of @cde:short:pl that are either performing the task or being idle. Executions are started at the same time within each group: for example, when measuring the execution time with 3 active @cde:short:pl, all 3 @cde:short:pl start the task at the same time.
...
From these results, we can make a number of observations. Firstly, for @cde:short:pl using pooled performance, we observe a correlation between the amount of idle neighbors and the reduction in execution time: the more neighbors are idle, the lower the execution time is. Secondly, for @cde:short:pl using static performance, we observe that the execution time is not impacted by the idle-active distribution: no matter the amount of idle neighbors, @cde:short:pl using static performance perform the task in a consistent time. Thirdly, we observe that when all @cde:short:pl are active, the execution time of both pooled performance and static performance is nearly equal.
...
In order to investigate the consistency of performance allocations, we will perform an experiment that simulates a consistently sized, distributed workload: each CDE in each pool will perform the task continuously, and the task is the same across all @cde:short:pl, namely writing 4 GiB of data to the disk. The simulation is ran continuously for 180 minutes, measuring the execution time of each execution.
...
Up until now, the previous sections of this chapter have presented experiment results with general observations. In this section, we will relate the experiment results to Uber's CDE, in order to understand what the real-world impact for the raw disk performance of Uber's @cde:short:pl may be.

=== Burstability
The first advantage of pooled performance is the ability to share resources for bursty workloads. When it comes to the utilization pattern of @cde:short:pl, it is commonly understood that they are bursty #cite(<potter-2024>). Typically, engineers will have their CDE open throughout the day, but the majority of this time is spent on idle tasks such as writing code or reading documentation. It is only when engineers actively compile projects or run test suites that @cde:short:pl require performance, and these tasks often have short durations.
...
In addition to the bursty utilization pattern, consider that Uber's @cde:short:pl are provisioned on a virtual machine holding up to 12 CDE deployments. Taking all factors into account, we understand the utilization pattern of @cde:short:pl is bursty and resources are shared between up to 12 deployments. Therefore, it is likely that individual @cde:short:pl would significantly benefit from pooled performance, as it is unlikely that a majority of the 12 deployments would be executing performance intensive tasks.
