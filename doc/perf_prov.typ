...
However, the consistency under pooled performance is perfectly acceptable for Uber's @cde:short:pl. @cde:short:pl are effectively development environments, and the difference in consistency would realistically not be perceived by a human engineer. The consistency of pooled performance resource allocations is adequate for @cde:short:pl as a use case.
...
To conclude, the pooled performance model of Persistent Disks is more advantageous for Uber's CDE on average. Resource pooling allows for significantly larger performance allocations for active @cde:short:pl, as their neighbors are likely idle. While static performance brings other benefits, such as resiliency against noisy neighbors and improved consistency of performance allocations, these other benefits are minimally useful for Uber's CDE. Although static performance offers less raw performance in an apples-to-apples comparison, it is important to note that the allocated performance using Hyperdisks is configurable: if additional raw performance is required, it can be provisioned at additional cost.
