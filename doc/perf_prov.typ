...
From the graph above, we can make a number of observations. Firstly, for CDE's using pooled performance (labeled "PD" for Persistent Disk), we observe a correlation between the amount of idle neighbors and the reduction in execution time: the more neighbors are idle, the lower the execution time is. Secondly, for CDE's using static performance (labeled "HD" for Hyperdisk), we observe that the execution time is not impacted by the idle-active distribution: no matter the amount of idle neighbors, CDE's using Hyperdisks perform this task in a consistent time. Thirdly, we observe that when all CDE's are active, the execution time of both pooled performance and static performance is nearly equal.

== Advantages of static performance provisioning
As described in the previous graph, performance pooling can allow for significantly improved performance for burstable workloads. While this seems like a significant advantage, it also introduces a lack of predictability and consistency when it comes to the availability of these resources. 

=== Improved predictability
Performance pooling results in a lack of predictability regarding the availability of performance resources. This is commonly known as the "noisy neighbor" problem within computing: suppose that 4 CDE's have to share the performance resources allocated to a given pool, but 1 CDE within this pool is executing long-running performance intensive tasks, effectively claiming all resources in the pool. In this situation, if a second CDE wants to perform a task, the performance scheduler needs to fairly re-distribute the performance resources within the pool, effectively deallocating resources from the first CDE and allocating them to the second CDE.

In order to investigate the fairness of the resource pooling algorithm, we will perform another experiment measuring raw disk performance using a synthetic benchmark. In this experiment, we will again set up two groups of 4 CDE's each, where the first group is using Hyperdisks and the second group is using Persistent Disks. The task that we will measure is writing 1 GiB of data to the disk. In order to measure fairness, these tasks will be performed with a varying amount of _noisy neighbors_. As previously described, a noisy neighbor is a resource consumer within the same pool that is already occupying performance resources prior to the start of the task execution. For this specific experiment, we will simulate a _noisy neighbor_ by instructing it to continuously write randomized data to the disk as fast as possible, effectively attempting to use as many performance resources as available.
...
In the graph above, we observe that noisy neighbors have a significant negative impact on execution time: as the amount of noisy neighbors grows, execution time is severely degraded, indicating that the performance redistribution algorithm for Persistent Disks is not sufficiently fair. In the case of 1 bad neighbor, the resource pool has enough resources for 4 CDE's in total. One of these CDE's is acting as the bad neighbor, and the other CDE is performing the measured task. Even in this case, we observe that the CDE performing the task was not allocated performance fairly, as it's execution time is slower than the group using static performance provisioning. On the other hand, the group using static performance provisioning is not impacted by noisy neighbors whatsoever: we observe that the execution time for Hyperdisks is nearly equal in all scenarios.
...
To demonstrate this difference across the two performance provisioning models, we will perform another experiment measuring raw disk performance using a synthetic benchmark. In this experiment, we will again set up two groups of 4 CDE's, where the first group is using Hyperdisks and the second group is using Persistent Disks. The task that we will measure is writing 4 GiB of data to the disk. We will instruct each CDE in each pool to perform this task continuously, measuring the execution time of each execution. 
