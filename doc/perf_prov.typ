...
=== Burstability
The first advantage of pooled performance is the ability to share resources for bursty workloads. When it comes to the utilization pattern of CDE's, it is commonly understood that they are bursty #cite(<potter-2024>). Typically, engineers will have their CDE open throughout the day, but the majority of this time is spent on idle tasks such as writing code or reading documentation. It is only when engineers actively compile projects or run test suites that CDE's require performance, and these tasks often have short durations.

As an example, consider the following utilization graph of one of Uber's CDE's:
...
In addition to the bursty utilization pattern, consider that Uber's CDE's are provisioned on a virtual machine holding up to 12 CDE deployments. Taking all factors into account, we understand the utilization pattern of CDE's is bursty and resources are shared between up to 12 deployments. Therefore, it is likely that individual CDE's would significantly benefit from pooled performance, as it is unlikely that a majority of the 12 deployments would be executing performance intensive tasks.
...
Using a Prometheus query, we can measure the allocated IOPS for each Persistent Disk in the region, and select metrics where the allocated IOPS are above a certain threshold. Considering that Uber's CDE's share 100.000 IOPS per resource pool, we define a noisy neighbor as an individual disk using 40% of this capacity, i.e. 40.000 IOPS. The full query is available in the Appendix (@prom_disks).
...
However, the accuracy of pooled performance is perfectly acceptable for Uber's CDE's. CDE's are effectively development environments, and the difference in consistency could not be perceived by a human engineer. The consistency of pooled performance resource allocations still performed consistent enough for CDE's as a usecase.

To conclude, while static performance does provision performance more consistently, this advantage is irrelevant to Uber's CDE's.
...
To conclude, the pooled performance model of Persistent Disks is more advantageous for Uber's CDE. Resource pooling allows for significantly larger performance allocations for active CDE's, as their neighbors are likely idle. While static performance brings other benefits, such as resiliency against noisy neighbors and improved consistency of performance allocations, these other benefits are minimally useful for Uber's CDE's. 
