...
The locally-attached disk is used to store the build output and the build cache. As mentioned in the introduction, a key characteristic of monorepos is that dependencies are cached on the disk. By storing this disk cache on a locally-attached disk, the @cde:short:pl can leverage the performance of locally-attached disks. Additionally, the lack of data persistence for locally-attached disks is acceptable, as it merely holds cached build output. It is important to note that locally attached disks are provisioned to virtual machines, and not instances of the application. Each virtual machine is provisioned with 6 TiB of local storage, and a given virtual machine holds around 12 instances of @cde:short:pl. Effectively, a group of 12 @cde:short:pl share 6 TiB of locally attached storage.
