...
The fundamental difference between local disks and network-attached disks is the way they interface with the virtual machine. Local disks have a direct hardware connection to the machine. Within the OSI model#footnote[#link("https://www.cloudflare.com/learning/ddos/glossary/open-systems-interconnection-model-osi/")[cloudflare.com/learning/ddos/glossary/open-systems-interconnection-model-osi/]], this is considered the lowest layer: the physical layer. On the other hand, network-attached disks interface through a network connection to the machine, which is the third layer of the OSI model. Google Cloud refers to locally-attached and network-attached disks as "temporary" and "durable" disks respectively #cite(<gcp-disk-overview>). The different attachment methods result in a number of different characteristics between the disks, which are highlighted as follows.

The first characteristic is performance. The performance of locally attached disks is an order of magnitude higher than network-attached disks, primarily due to the fact that they are attached at different layers within the OSI model. Specifically, local disks can scale up to 3,2 million IOPS and 12.480 MiBps of throughput #cite(<gcp-lssd-iops>), whereas network-attached disks can only scale up to 120.000 IOPS and 2.200 MiBps of throughput #cite(<gcp-pd-iops>).

The second characteristic is data persistence. Local disks are ephemeral, meaning the data stored on them is discarded if the virtual machine they’re attached to is restarted. This makes local disks more suited for temporary storage use-cases, such as caching. On the other hand, network-attached disks are able to persist their data, making them more suited for long-running use-cases, such as databases #cite(<gcp-disk-overview>). 
...
The network-attached disk is used as the primary form of storage for the user's files. Considering that local disks are ephemeral, they are not suited to store user files, as they would not be persisted across restarts. Specifically, the network-attached disk is used for the _home directory_ of a @cde:short. This includes various things, such as the source code for all repositories the user has cloned, or any local databases or configuration files the user needs. Each @cde:short is provisioned with 256 GiB of capacity.

The locally-attached disk is used to store the build output and the build cache. As mentioned in the introduction, a key characteristic of monorepos is that dependencies are cached on the disk. By storing this disk cache on a locally-attached disk, the @cde:short:pl can leverage the performance of locally-attached disks. Additionally, the lack of data persistence for locally-attached disks is acceptable, as it merely holds cached build output. It is important to note that locally attached disks are provisioned to virtual machines, and not to instances of the application. Each virtual machine is provisioned with 6 TiB of local storage, and a given virtual machine holds around 12 instances of @cde:short:pl. Effectively, a group of 12 @cde:short:pl share 6 TiB of locally attached storage.
...
In addition to the ability to share capacity, the cloud provider states that Hyperdisk Storage Pools use various data reduction technologies to increase storage efficiency #cite(<gcp-hd-pool-features>), potentially data deduplication and compression. The cloud provider does not provide in-depth detail regarding how these data reduction algorithms are implemented, nor what the expected reduction ratio is. Therefore, these claims are further investigated in @disk_cap.

== Provisioning Hyperdisks for the application <section_prov_hd>
The application currently provisions Persistent Disks for its network-attached storage use-cases. This section provides a brief overview of the steps taken for the application to provision Hyperdisks instead of Persistent Disks.

=== How disks are provisioned
The application is built on Kubernetes, an orchestration system for containerised applications#footnote[#link("https://kubernetes.io")]. Kubernetes is effectively an abstraction layer on top of the cloud provider used by the application. Practically, this means that the application provisions Persistent Disks by creating Kubernetes resources, and Kubernetes further interfaces with the cloud provider's API to provision the disk. The two relevant Kubernetes resources are the `StorageClass` resource and the `PersistentVolumeClaim` resource.

The `StorageClass` resource defines a specific class of storage that the application can provision. Storage classes define the type of underlying disk to provision, for example Persistent Disks or Hyperdisks. Additionally, they define related properties, such as backup policies or encryption strategies.
...
Currently, the application has a static `StorageClass` resource that specifies the Persistent Disk type, which is available in #link(<k8s_pdsc>)[Appendix B] for reference. As new instances of the application are provisioned, a `PersistentVolumeClaim` resource is created. An example of this resource is available in #link(<k8s_pdpvc>)[Appendix C] for reference.

=== Provisioning Hyperdisks <provisioning_hd>
In order to implement Hyperdisks, a new storage class is created. This storage class specifies that the underlying disk type is a Hyperdisk. Additionally, the storage class defines a number of properties specific to Hyperdisks. The performance resources are defined, meaning the number of IOPS and the throughput in MiBps. The definition of the resource is available in #link(<k8s_hdsc>)[Appendix D] for reference.

If Hyperdisk Storage Pools are used, the storage class for Hyperdisks must additionally specify the name of the storage pool that disks using the storage class should be attached to, by defining the `storage-pool` property. An example of this change is available in #link(<k8s_hdsc_pool>)[Appendix E] for reference.

Additionally, in this case, the storage pool itself must be provisioned. As Hyperdisk Storage Pools are a concept specific to the cloud provider, they cannot be provisioned through Kubernetes. Hyperdisk Storage Pools can be created using the `gcloud` command line tool, with the exact command available at #link(<gcloud_pool>)[Appendix F]. 

Once the storage class is created, the PersistentVolumeClaims that the application creates can specify the new storage class by changing the `storageClassName` property. An example of this change is available for reference at #link(<pvc_change>)[Appendix G]. 
