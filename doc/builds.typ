...
== Conclusion
An answer to the sub-question _"How is build performance affected by migrating from Persistent Disks to Hyperdisks?"_ can now be provided. In this chapter, an experiment comparing three disk configurations was performed, namely a Persistent Disk configuration with 38.640 IOPS, a PD-equivalent Hyperdisk configuration with 9.660 IOPS, and a minimum Hyperdisk configuration with 3.000 IOPS. For each of the configurations, the build time was measured for each one of Uber's CDE-supported monorepos, namely the Go, Web, Python, Android and Java monorepos.
