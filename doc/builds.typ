...
The first configuration is the Persistent Disk configuration. For this configuration, we intentionally want to measure the resource pooling advantage of Persistent Disks described in @perf_prov. Specifically, we will provision a pool of 4 @cde:short:pl that each have a 256 GiB Persistent Disk, and only one of these @cde:short:pl will be used for measurement. By using the formulas defined in @perf_model, we can calculate the performance resources for this pool, which is 38.640 IOPS. Effectively, the Persistent Disk configuration is a CDE with a total resource pool of 38.640 IOPS.
...
In order to provision the @cde:short:pl with different disk configurations, we follow the instructions defined in @provisioning_hd.
