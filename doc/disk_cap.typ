...
Thirdly, an appropriate autoscaling utilization target is defined. The cloud provider suggests a utilisation target of 90%. One could argue that this utilisation target can be optimised further, considering that Uber's CDEs operate at a large scale, making it unlikely that individual CDEs could cause utilization spikes significant enough to cause disruptions. However, the impact of these disruptions would be significant for Uber's CDEs, as they are considered a critical service. Therefore, the suggested utilisation target of 90% remains appropriate.
...
== Data reduction
=== Problem context
Utilization aside, another characteristic of Uber's CDEs is duplicate data. As previously mentioned, Uber has adopted the concept of monorepos, which are a development strategy where multiple projects are located within the same repository. These monorepos can grow to up to 40+ GiB in size, and each CDE is pre-configured with a monorepo cloned, meaning the same large monorepos are stored across thousands of disks.
