...
While thin provisioning significantly improves utilization, there are constraints to be aware of. A direct consequence of overprovisioning is the fact that provisioned storage may not be physically available as utilization grows. Suppose that 100 CDE's are provisioned with 256 GiB of capacity each, totaling 25 TiB of provisioned capacity, and the average utilization across the CDE's is 20%. In this case, only 5 TiB of storage is utilized, meaning a storage pool with 5 TiB of physically provisioned capacity would fit. However, when the utilization of the CDE's grows, there would not be any physical storage available, causing disk operations to fail. In this scenario, the storage pool will automatically provision additional physical storage. As this is not an instant operation, it is recommended to configure an autoscaling utilization target: this target percentage defines when the storage pool automatically provisions additional capacity. For example, with a utilization target of 90%, the storage pool will have time to provision the additional capacity, reducing the likelyhood of disruptions. 
...
Firstly, the current total provisioned capacity is calculated. For the purpose of this research, this is defined hypothetically. In this hypothetical scenario, Uber currently provisions 1000 CDE's with 256 GiB each, totaling 500 TiB provisioned. 

Secondly, the current average utilization for Uber's CDE's is calculated using disk utilization metrics. This utilization is 28,35%. 

Thirdly, an appropriate autoscaling utilization target is defined. The cloud provider suggests a utilisation target of 90%. One could argue that this utilisation target can be optimised further, considering that Uber's CDE's operate at a large scale, making it unlikely that individual CDE's could cause utilization spikes significant enough to cause disruptions. However, the impact of these these disruptions would be significant for Uber's CDE's, as they are considered a critical service. Therefore, the suggested utilisation target of 90% remains appropriate.
...
== Data reduction
=== Problem context
Utilization aside, another characteristic of Uber's CDE's is duplicate data. As previously mentioned, Uber has adopted the concept of monorepos, which are a development strategy where multiple projects are located within the same repository. These monorepos can grow to up to 40+ GiB in size, and each CDE is pre-configured with a monorepo cloned, meaning the same large monorepos are stored across thousands of disks.
...
The results from the experiment for block-based duplication indicate that it is unlikely that Hyperdisk Storage Pools apply deduplication. Recall that for the 8192-byte repeating pattern, there was no data reduction. If file-level deduplication was applied, data reduction would have been observed, as the same 8 GiB file was duplicated across 8 disks within the pool. If block-level deduplication was applied, data reduction would have been observed, as the same 8192-byte pattern was repeated into an 8 GiB file ($128 dot 1024 dot 8$ times).
...
These results further confirm that Hyperdisk Storage Pools do not seem to apply deduplication. Observe that as the number of dataset instances increases, the compressed size grows linearly, meaning reoccuring data is not being deduplicated. Despite the lack of deduplication, the 2.31:1 compression ratio is still impressive for a real-world dataset.

=== Real-world evaluation
The two sections above have investigated the potential storage efficiency improvements using synthetic experiments. In addition to these synthetic experiments, the most accurate estimation for the improvements can be determined by provisioning a storage pool with copies of production instances of Uber's CDE's. Such an experiment is quite feasible, as Uber's CDE system creates automatic snapshots of provisioned disks, and these disks can be cloned into a new storage pool with a number of simple scripts.

For simplicity, this experiment was executed in only one datacenter of Uber's CDE infrastructure. Considering the amount of CDE's globally, the size of this set of CDE's can accurately represent the storage patterns for the entire fleet.
...
+ Select all CDE's in the region that were used in the past 14 days
+ For each CDE, select it's most recent snapshot
...
The result of this experiment are as follows:
#table(
  columns: 2,
...
From these results, the total capacity reduction yielded from Hyperdisk Storage Pools can be calculated. Without storage pooling, the total capacity of this set of CDE's is 187,75 TiB. When mounting each of the disks, 57,72 TiB of data written to the disks was observed. At the storage pool level, the amount of used capacity reported post-compression is 53,60 TiB, meaning that the data reduction resulted in an additional 7.13% reduction of capacity usage (1,08:1). Lastly, as the 53,60 TiB in use by the storage pool should be a 90% utilization target, the final storage pool size would be $53","60 dot 100/90 = 59","56 "TiB"$. Effectively, the total capacity reduction is $1 - (59,56)/(187,75) = 68","28%$. 

== Conclusion
An answer to the sub-question of this chapter, _"How do Hyperdisk Storage Pools affect disk capacity utilization compared to Persistent Disks?"_, can now be provided. In this chapter, two Hyperdisk Storage Pool features were identified as being relevant to disk capacity utilization improvements, and these features were thoroughly investigated. 
...
Lastly, a subset of production CDE's were cloned and provisioned into a Hyperdisk Storage Pool, in order to approximate the real-world capacity reduction for Uber's CDE fleet. For this subset, Hyperdisk Storage Pools reduced the total purchased capacity from 187,75 TiB to 59,56 TiB, which is a 68,28% reduction. This observed reduction aligns with the expected reduction of 68,5% that was calculated before.

As mentioned in the introduction of this chapter, Uber's CDE's are provisioned with various components, including CPUs, RAM, GPUs and storage. A significant share of the total cost of these components is attributed to storage. Therefore, reducing the total purchased storage capacity by 68,28% would be a significant improvement in cost efficiency for Uber's CDE's.
